...
class SavePath(object):
    ...
    def sub_callback(self, msg):
        ...
        self._pose = str(self._temp.pose.position.x) + ' ' + str(self._temp.pose.position.y) + ' ' + str(
            self._temp.pose.position.z)
        self._currentposition = np.array(
            [self._temp.pose.position.x, self._temp.pose.position.y, self._temp.pose.position.z])
        ...
        if set_local_pos_ref:
            ...
            with open('localPoseStamped.txt', 'w') as file:
                ...
                while not rospy.is_shutdown():
                    file.write(str(self._currentposition) + ' \n')
                rate.sleep()
